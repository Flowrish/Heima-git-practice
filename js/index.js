...
function genderRender(salaryData){
 ...
 function provinceRender(provinceData){
  ...
  const dataList = [
   ...
   dataList.forEach(ele => {
    const res= provinceData.find(item => {
      return item.name.includes(ele.name)
    })
     console.log(res);
     if(res) ele.value=res.value
  })
  const option = {
    title: {
      text: '籍贯分布',
